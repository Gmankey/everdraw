...
- Read API: `getTwabBetween(vault, account, startTime, endTime)`, `getTotalTwabBetween(vault, startTime, endTime)`, and an explicit delegated-balance read for the sponsor sink (`getDelegateTwabBetween(vault, delegate, startTime, endTime)` or equivalent). Winner odds use participant TWAB only: `totalPrincipalTWAB - sponsorDelegateTWAB`. Sponsor-fee attribution (§5.3) uses the full principal basis: `sponsorDelegateTWAB / totalPrincipalTWAB`.
...
- Amounts in leaves are **fixed token amounts, fully escrowed before any root can be proposed** *(amended per M0 findings 7 and builder re-review)*: when `startDraw` snapshots the prize legs, DrawManager also withdraws the yield leg out of the strategy into ClaimManager as the raw deposit asset (native MON, paid at claim via low-level call — a failing native send defers like any other leaf), and reward legs are already held raw since funding. If the yield-leg withdrawal cannot be escrowed, the draw does not advance to randomness/root proposal; it stays in an operationally alarmed state while deposits/withdrawals/emergency exits remain governed by the vault rules. A root may only be proposed for already-escrowed legs, so after proposal no claim depends on strategy share price, rebasing behavior, or venue liveness — drift between proposal and claim is structurally impossible, not "absorbed." Yield accruing after the snapshot belongs to the next draw. (Storing proportions instead was considered and rejected: it makes leaves non-verifiable against `totalPayout` and complicates the challenge check.)
...
- **6a:** per-vault `feeBase` flag: `TOTAL_PRIZE` (yield + reward legs) or `PARTICIPANT_YIELD_ONLY` (reward-token legs and sponsor-attributable yield exempt). *(Amended per M0 finding 4:)* sponsor-attributable yield is **time-weighted, not snapshotted**: `grossYield × sponsorDelegateTWAB / totalPrincipalTWAB`, both measured over the draw period via TwabController's explicit delegated-balance read (§3.1). Winner odds still exclude sponsor-delegated balances; fee attribution needs the full principal basis so sponsor and participant yield add up to 100%. A snapshot basis would let a sponsor deposit/withdraw at the draw boundary to distort fee attribution; under TWAB a boundary balance has ~zero weight, so the same property that kills depositor timing attacks kills this one.
