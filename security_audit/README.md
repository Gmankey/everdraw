...
**For the current production review, read [`AUDIT_REPORT_V3_2026-05-28.md`](./AUDIT_REPORT_V3_2026-05-28.md).** That is the authoritative review against the production contracts deployed on Monad mainnet.
...
|---|---|---|---|
| [`AUDIT_REPORT_V3_2026-05-28.md`](./AUDIT_REPORT_V3_2026-05-28.md) | 2026-05-28 | `TicketPrizePoolShmonV3` at commit `186f1ad` — the production contract used by Vault A V3 (`0x8F36aaAD...`) and the scheduled Vault B V3 | **Current** — no HIGH/MEDIUM findings at confidence ≥ 8 |
...
- **Operators / new contributors:** start with the V3 report. The V1-era report is helpful for context on why decisions were made but does not describe live risks.
- **Third-party auditors:** read both. The historical report establishes the threat model lineage; the V3 report establishes the current contract surface. Cross-check the "what changed" banner against the ADRs that are cited.
- **Users:** the V3 report's executive summary in §1 is the high-confidence statement of the current protocol's security posture. The full report is for engineers.

## Future audits

A third-party human audit is budgeted and planned before TVL scales beyond bootstrapping levels (>1000 MON in any single vault). When complete, that report will be added to this directory and the V3 internal report will move into a "Past internal reviews" subsection rather than being promoted as the canonical reference.
