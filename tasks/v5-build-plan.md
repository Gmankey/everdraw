...
**M8 — Testnet soak.** Full stack (contracts, keeper, watcher, indexer, frontend) on testnet for ≥3 full draw cycles at an accelerated cadence, including one deliberately-injected bad root (veto drill) and one keeper outage (permissionless-fallback drill). *Gate (hardened per M0 finding 6 — these are launch-gating, not best-effort):* all drills pass; veto runbook written AND executed by the operator personally on testnet; dead-man heartbeats live on keeper and watcher via an independent channel; watcher hosted off-Fly; draw timing pinned inside operator waking hours; deploy/draw-ops/strategy-swap runbooks written.

**M9 — Mainnet + cutover.** Operator/builder execute deploy per runbook (PM writes instructions only). Owner stays single Ledger per Q7; deposit cap set to the operator-approved launch value before deposits open. Frontend "Move to V5" flow. Merkl notified pre-cutover. *Gate (rule #6; shMON elevated per M0 finding 10 — this exact surface drifted before):* live frontend bundle serves V5 addresses; a real **native-MON deposit AND a real direct-shMON deposit** → draw → prize → claim observed on-chain end-to-end; the Move-to-V5 two-tx migration flow executed against production; Merkl points confirmed flowing; deployments/ADR records updated in the same change.
