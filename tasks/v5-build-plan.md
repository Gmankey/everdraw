...
| Q1–Q7 launch parameters | **Open** — see below; B3 blocker |
| Audit firm/budget (B1) | **Open** — operator; long lead time, start now |
...
## Operator decisions needed (ADR-0036 §10)

| # | Decision | Blocks | PM recommendation on record |
|---|---|---|---|
| Q1 | Launch draw cadence | M3 | Weekly at launch, shorten later via the tunable |
| Q2 | Winner count + tier split per draw | M3 | — (product call) |
| Q3 | Min deposit | M2 | — |
| Q4 | Challenge window length | M3 | 8 hours |
| Q5 | Proposer bond in V5.0? | M3 | Veto-cooldown-only at launch; bond as fast-follow |
| Q6 | Audit firm + budget | M7 | Engage during M1, not at code-complete |
| Q7 | Multisig before V5.0 mainnet | M9 | Yes (ADR-0031; owner key now carries veto + allowlist powers) |
...
**M7 — External audit (B1).** Scope: all V5.0 contracts + the algorithm spec. The off-chain reference implementation goes to the auditor as context. *Gate:* report delivered; criticals/highs fixed and re-reviewed.
...
| Audit lead time slipping the schedule | Operator | engage at M1 (Q6) |
