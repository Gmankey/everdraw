...
**Builder action:** `.github/workflows/v5-watcher.yml` runs every five minutes on UAT, leaving response margin inside its 15-minute challenge window. It rebuilds the input for each proposed root directly from chain events using `scripts/draw/write-watch-inputs.mjs`; it does not consume the keeper's local `draw-inputs/` directory or share Fly storage. The configured logs-RPC secret is used instead of a hardcoded public endpoint, and an OK heartbeat is emitted only after the persistent cursor reaches chain head.
