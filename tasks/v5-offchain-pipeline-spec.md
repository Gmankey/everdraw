...
**Step 5 — leaves and tree** *(amended per M0 findings 2–3)*. One leaf per `(position, leg)` aggregated by `(account, token)`, then assigned a sequential `leafIndex` (0-based, in canonical account-then-token order): `leaf = keccak256(abi.encode(LEAF_DOMAIN, distributionId, leafIndex, account, token, amount))` where `distributionId = keccak256(DrawManager address, drawId)`. The explicit `leafIndex` uniquely identifies every leaf (duplicate winners, fee+winner overlap, multi-token) and keys the claimed-bitmap and per-leaf deferred records. Tree: sorted-pair keccak merkle (OpenZeppelin `MerkleProof`-compatible), leaves sorted ascending. Output: `root`, `leafCount`, `totalPayout` per token (must equal the snapshotted legs exactly — the contract enforces this).
