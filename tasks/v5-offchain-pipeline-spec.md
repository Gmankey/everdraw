...
**Step 1 — account set.** Every address with ≥1 Merkl-shaped `Deposit` event on the V5 vault up to `end`, enumerated and then filtered to `twab > 0` (below). Native MON deposits and direct shMON deposits both emit the same `Deposit(account, assetValue)` event per ADR-0036 §8; there is no separate Merkl-visible `DepositShmon` event. Sponsor addresses are excluded by construction: their balances are delegated to the sponsor sink and subtracted from the winner-odds denominator. Canonical ordering: ascending address (byte order).

**Step 2 — per-account TWAB.** `twab(a) = TwabController.getTwabBetween(vault, a, start, end)` evaluated at `seedBlock` state. Implementations MAY reconstruct from events instead of state reads, but the state read is the reference. Units: wei (average balance over the period, floor). *(Dependency note per M0 finding 5: this read requires archive-state access at `seedBlock`, and Step 1 requires the full event log — verifiers need an archive node or trusted index; see ADR-0036 §7.2 archive-RPC row. Event-ingestion reorg policy: ingest only finalized blocks; on detected reorg below the finality assumption, re-sync from the last finalized checkpoint and recompute from scratch — never patch incrementally.)*

**Step 3 — sampling.** Build the cumulative participant line `C_i = Σ twab(a_0..a_i)` over the canonical ordering; `T = C_last` (must equal `getTotalTwabBetween(vault, start, end) - getDelegateTwabBetween(vault, SPONSOR_DELEGATE, start, end)` or the equivalent explicit sponsor-sink read; mismatch = abort, alarm). For position `j` in `0..K-1`: `r_j = uint256(keccak256(abi.encode(seed, drawId, j))) mod T`; winner = the account whose cumulative interval contains `r_j` (binary search). Sampling is with replacement (one account can win multiple positions).
...
**Step 5 — leaves and tree** *(amended per M0 findings 2–3 and builder re-review)*. Create one leaf per payout item: each `(position, leg)` winner payout, plus fee-recipient leaves. Do **not** aggregate duplicate account/token payouts before assigning identity; duplicate wins, fee+winner overlap, and multi-token payouts remain distinct leaves. Assign a sequential `leafIndex` (0-based) in canonical payout order: draw winner positions in ascending `position`, legs in configured order, then fee leaves in configured recipient/leg order. `leaf = keccak256(abi.encode(LEAF_DOMAIN, distributionId, leafIndex, account, token, amount))` where `distributionId = keccak256(DrawManager address, drawId)`. The explicit `leafIndex` uniquely identifies every leaf and keys the claimed-bitmap and per-leaf deferred records. Tree: sorted-pair keccak merkle (OpenZeppelin `MerkleProof`-compatible), leaves sorted ascending after hashing. Output: `root`, `leafCount`, `totalPayout` per token (must equal the snapshotted and escrowed legs exactly — the contract enforces this).
...
- New event handlers: `Deposit`/`Withdraw` for Merkl-visible participant balances (including direct shMON deposits, valued as ADR-0036 §8 specifies), `SponsorDeposit`/`SponsorWithdraw` for zero-odds sponsor balances, `BoostDeposit`/`BoostWithdraw` for the distinct ADR-0040 booster points campaign, `DrawStarted`/`SeedReceived`/`RootProposed`/`RootVetoed`/`RootFinalized`, `Claimed`/`ClaimDeferred`/`DeferredClaimed`, cap/threshold/config-change events. V4 handlers stay (both generations indexed during coexistence). Booster events must not be folded into participant odds-points.
