...
- New event handlers: `Deposit`/`Withdraw` for Merkl-visible participant balances (including direct shMON deposits, valued as ADR-0036 §8 specifies), `SponsorDeposit`/`SponsorWithdraw` for zero-odds sponsor balances, `DrawStarted`/`SeedReceived`/`RootProposed`/`RootVetoed`/`RootFinalized`, `Claimed`/`ClaimDeferred`/`DeferredClaimed`, cap/threshold/config-change events. V4 handlers stay (both generations indexed during coexistence).
