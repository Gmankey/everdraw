# Everdraw — Nitro Accelerator Application Answers (v3)
...
But the larger bet is B2B2C. Protocols will spend millions on airdrops and points programs over the next 12 months — incentive models with a proven retention problem: 90%+ farmer churn, mercenary capital, and one-time engagement. Everdraw gives them an alternative: replace your points program with a points lottery. Same budget, distributed as a prize instead of a linear drip. Users participate for the chance to win big, come back weekly, and post when they win. Protocols get the dopamine-driven engagement loop that betting markets are currently monopolizing — without users losing money.

A protocol funds a campaign through our CampaignManager with a simple treasury transfer. No contract changes on their side. No audit required. They define eligibility, set the prize, and Everdraw handles draws, winner selection, and claims.

The product isn't hypothetical. 39/39 tests passing. Keeper bot live on systemd. Full round lifecycle validated on Monad testnet. Frontend deployed.

Phase 2 ships TWAB and the CampaignManager. The endgame: Everdraw becomes the prize layer behind every "deposit and win" campaign across the Monad ecosystem — the infrastructure through which protocols replace one-time token dumps with recurring engagement that actually sticks.
...
Everdraw doesn't primarily compete with PoolTogether or Pendle. Pendle serves sophisticated yield traders who want predictable, priced yield exposure — that's not our user. Our user is the casual holder who wants excitement without complexity, and the protocol that wants recurring engagement instead of a one-time token dump. Pendle serves yield traders. Everdraw serves everyone else — and that's a bigger market.

Our real competition is whatever protocols currently spend on user acquisition: airdrops (one-time, 90%+ farmer churn), liquidity mining (mercenary capital that leaves), and points programs (increasing fatigue). Prize campaigns offer the same budget deployed as recurring engagement instead of one-time extraction.

**8. Natural expansion into the points ledger category.**

Protocols like Merkl handle campaign creation, eligibility tracking, and reward distribution for DeFi protocols. Everdraw's CampaignManager does the same — with a lottery mechanic on top that purely linear distribution systems can never add without rebuilding their core model. Linear reward distribution has no viral moment. A prize draw does.

The natural Phase 3 expansion: Everdraw adds continuous points tracking for all campaign participants, not just winners. A hybrid model — earn points for every eligible action, plus a chance to win the jackpot — is more engaging than anything the category currently offers and something protocols would immediately understand. The campaign infrastructure we're building today is the foundation of a points layer that out-engages every existing distribution tool.
...
Everdraw is pre-mainnet. We don't have live retention metrics yet — but we have committed partners, ecosystem validation, and a technically complete product.

**Committed B2B design partner: ShMonad**
ShMonad — Monad's largest LST — has formally committed to Everdraw as a design partner and endorsed the protocol. They will be the first protocol to run a campaign through the CampaignManager, funding a branded prize draw for their stakers. This is a formal commitment, not a warm relationship. ShMonad holds the largest staked MON position in the ecosystem. Their endorsement is both our first distribution channel and the proof point for the B2B2C thesis.

**Ecosystem validation from Monad's core team**
Everdraw was shared with Monad's dev team for a roadtest prior to this application. Feedback was strongly positive. An unsolicited endorsement from the chain's core team before mainnet launch is a meaningful signal.

**Early product usage**
A small group of early testers have run full round lifecycles on the live testnet dApp — depositing, buying tickets, and completing draws end-to-end. No blocking UX issues surfaced. The product is ready.

**Technical execution as a demand signal**
39/39 tests passing. Full lifecycle validated on-chain. Keeper bot proven through 24h+ burn-in with zero missed rounds. Built solo — Solidity, React, systemd. This isn't "in development." It exists.

**Community is the active next step**
Discord, Twitter, and waitlist launch immediately post-application. The first public mainnet draw is the ignition moment — every winner is a tweet, every draw is a recurring content event. We are not pre-product. We are pre-audience.
